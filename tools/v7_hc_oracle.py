...
"""Independent float32 HyperConnection equations for V7 localization.
...
`expf`/`sqrtf` from libm. Every multiply, add, divide, and normalization step is
rounded to IEEE float32 in the same order as `deepseek_v4_mhc_ref.c`.

Why this is separate from the earlier Gate-H Python oracle: Gate H's fixture
never placed an HC-pre value exactly on a BF16 boundary. Chaining the real
layer-3 output into layer 4 did. Python's ordinary double-precision reductions
were close numerically but one BF16 wrong. V7 needs the *runtime dtype contract*,
not extra oracle precision.
"""
