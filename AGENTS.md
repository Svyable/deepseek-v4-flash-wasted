...
- Gate V1 is **half satisfied**: public number-format conformance and local indexing/matvec consistency pass, but official DeepSeek nibble order, scale direction, scale layout and oracle projection agreement remain unverified.
...
The public-format half is implemented in `src/quant/`; DeepSeek-specific convention reconciliation remains part of Gate V1.

### 6. Placement must not change numerics
...
### 10. Encoder semantics are part of model correctness
...
1. inventory tooling exists; real Gate V0 input is blocked in the original environment;
2. official reference/oracle harness is the next dependency once reference access exists;
3. scalar E2M1/UE8M0/E4M3 paths are implemented and public-format-tested; finish Gate V1 against official conventions before SIMD;
4. pass Gate V2 with one official quantized linear projection;
5. define/write the DeepSeek-specific container only after real tensor names/layouts are known;
6. implement mHC;
7. implement attention/compression/indexer;
8. implement routing/shared/routed MoE;
9. prove one layer, then multi-layer, then final logits;
10. port encoding/parser;
11. integrate CLI/server;
12. profile and optimize;
13. add DSpark only after the base path is stable.
...
The highest-leverage next action is authorized access to the pinned official Tier-0/Tier-1 files described in `docs/REFERENCE_ACCESS.md`; a full weight download is not required to answer most remaining V0/V1 convention questions.