...
"""Require the independent F32 HyperConnection oracle to match C bitwise."""
...
import deepseek_v4_hc_oracle as oracle  # noqa: E402
...
def main():
    ...
    py_pre, py_post, py_comb, py_mixes = oracle.params(x, fn, scale, base, dim=DIM)
    py_y = oracle.pre_y(x, py_pre, dim=DIM)
    py_after = oracle.post_y(branch, x, py_post, py_comb, dim=DIM)

    with tempfile.TemporaryDirectory(prefix="gate-h-hc-oracle-") as td:
    ...
    print("PASS Gate H independent F32 HyperConnection oracle matches C bit-for-bit")
